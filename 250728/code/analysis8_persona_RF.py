...
# --- Step 3: RandomForestモデル学習、精度検証、特徴量重要度の算出 ---
all_results_rf = {}
...
for group_name, target_cols in target_info.items():
    ...
    all_results_rf[group_name] = group_results

# --- Step 4: 結果の表示 ---
for group_name, results_dict in all_results_rf.items():
    print(f"\n===== 貢献度・精度分析 (RandomForest): {group_name} の回答 =====")
    if not results_dict:
...
for group_name, importances_dict in all_results_rf.items():
...
output_dir = '250728/fig/feature_importance_plots_rf'
...
print(f"\nRandomForestによる特徴量の重要度グラフを '{output_dir}' フォルダに個別に保存しました。")