...
# --- Step 3: RandomForestモデル学習とSHAP値の計算 ---
all_results_rf_shap = {}
print("SHAP analysis with RandomForest started. This may take a few minutes...")
...
for group_name, target_cols in target_info.items():
    ...
    all_results_rf_shap[group_name] = group_results

print("\nRandomForestによる分析が完了しました。")

# --- Step 4: SHAP サマリープロットの個別出力 ---
output_dir_shap = '250728/fig/feature_importance_plots_rf_shap'
os.makedirs(output_dir_shap, exist_ok=True)

for group_name, results_dict in all_results_rf_shap.items():
    if not results_dict:
        continue
    for target_col, results in results_dict.items():
        question_num = target_col.split('_')[1]
        plt.figure()
        title_text = (f"SHAP Values (RandomForest): {group_name} - Q{question_num}\n"
        ...
        if results['is_binary']:
            # 2値分類の場合、SHAP値はPositiveクラス(1)に対するものだけプロット
            # shap.summary_plotは自動でこれを処理してくれる
            shap.summary_plot(shap_values_to_plot[:,:,1], features=results_dict[target_col]['shap_explanation'].data, show=False)
        else:
...
print(f"\nRandomForestによるSHAP貢献度グラフを '{output_dir_shap}' フォルダに個別に保存しました。")