...
# Your Preference
True Preference List: {preference}
...
3. If an agreement is reached (Job Seeker sends [APPLY] and Company sends [ACCEPT]), a match is considered established, and subsequent negotiations continue among the remaining agents excluding these two.

The simulation ends when all Job Seekers and Companies are matched, or when round 100 is reached.
...
- If the Company is low on your list, consider compromising to avoid the risk of not matching with anyone.
...
{{
  "thought_process": "Internal thoughts considering the opponent's rank, market situation, and history",
...
# Current Situation
Current round: {round_number}
List of Job Seekers not yet matched: {active_jobSeeker}
Matched Job Seeker so far: {matched_jobSeeker_list}
Remaining Quota: {quota_current}
Target Job Seeker for this round: {target_jobSeeker}
Message from the target Job Seeker in this round: {current_message_from_jobSeeker}
...
- If target Job Seeker said [TALK], you MUST use [TALK].
