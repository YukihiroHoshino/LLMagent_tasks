...
In each round, Students are randomly paired with High Schools to engage in conversation.
...
1. The Student sends a message. Along with the conversation, one of the tags [APPLY], [TALK], or [WITHDRAW] must be specified.
...
3. If an agreement is reached (Student sends [APPLY] and High School sends [ACCEPT]), a match is considered established, and subsequent negotiations continue among the remaining agents excluding these two.
...
# Full History (All past interactions with various Students)
{full_history}

# Current Situation
Current round: {round_number}
List of Students not yet matched: {active_jobSeeker}
Matched Student so far: {matched_jobSeeker_list}
Remaining Quota: {quota_current}
Target Student for this round: {target_jobSeeker}
Message from the target Student in this round: {current_message_from_jobSeeker}

# Task
Make a decision through dialogue with the target Student.
- If the Student is high on your list, appeal actively.
- If the Student is low on your list, consider compromising.
- If the target Student is NOT on your priority list, you MUST NOT accept.

1. Reply to the message from the target Student.
