...
if __name__ == "__main__":
    ...
    for i in range(1, NUM_TRIALS + 1):
        ...
        for agent in sim.seekers:
            # 正直申告の判定
            # エージェントがsubmitted_list（提出したリスト）を持っていると仮定
            # 持っていない場合は安全策としてFalseあるいは判定不可とする
            submitted_list = getattr(agent, "submitted_list", [])
            true_pref = agent.preferences
            
            is_honest = (submitted_list == true_pref)
            if is_honest:
                trial_honest_agents += 1
        ...
        trial_stats.append({
            ...
            "honest_ratio": trial_honest_agents / total_agents if total_agents > 0 else 0
        })
    ...
    df_summary = pd.DataFrame([{
        ...
        "Avg_Honesty_Rate": avg_honest_rate,
