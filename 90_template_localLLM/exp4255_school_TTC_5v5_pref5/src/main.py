...
from algorithm_engine import run_ttc_algorithm
...
• Simultaneously, each High School identifies the Student with the highest priority (based on the High School's internal standards) among all Students. 
• The computer looks for a "closed loop" where applications and identifications match. This occurs in two cases: 
1. A Student applies to a High School, and that High School identifies the same Student. 
2. A chain is formed (e.g., Student A applies to School X, School X identifies Student B, and Student B applies to School Y... eventually leading back to a High School that identifies Student A). 
• Students involved in such a closed loop are permanently admitted to the High School to which they sent their application. 
• The admitted Students and the corresponding seats at the High Schools are removed from the procedure.

Step 2 
• For each Student who was not admitted in the previous step, an application is sent to the High School that they ranked highest among the High Schools that still have quotas. 
• Each High School with quotas identifies the Student with the highest priority among the Students who have not yet been admitted. 
• As in the previous step, the computer looks for closed loops where applications and identifications match or form a chain. 
• The Students involved in these loops are permanently admitted to the High School they applied to, and they are removed from the procedure together with the filled seats.

Following steps 
• The procedure continues according to the same rules.

Final Step
• The procedure ends when all Students are admitted or all quotas are filled.
"""
