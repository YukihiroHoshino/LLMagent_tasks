...
Ordering Phase
• At the beginning, the computer assigns a random serial order to all Students (e.g., 1st, 2nd, 3rd...). This order is determined purely by chance and is unrelated to the High Schools' internal standards.

Selection Procedure
• The computer calls Students one by one according to their assigned Serial Order.

Step 1
• The Student with the 1st Serial Order is assigned to the High School ranked highest on their "Choice Ranking List".

Step 2
• The Student with the 2nd Serial Order is assigned to the High School ranked highest on their "Choice Ranking List", strictly among the High Schools that have not yet been taken by the previous Student.

Following steps
• The procedure continues sequentially. Each Student is assigned to their highest-ranked High School that is still available (i.e., not taken by Students with an earlier Serial Order).
• If all High Schools on a Student's list are already taken, they remain unmatched.

Final Step
• The procedure ends after the last Student in the Serial Order has been processed. All assignments are final.
"""
