...
Part 1
Step 1
• For each Parent, an application is sent to the Nursery that they ranked first on their "Choice Ranking List".
• If a Nursery receives only one application, the Parent is temporarily admitted.
• If a Nursery receives more than one application, the Parent with the highest priority (based on the Nursery's internal standards) is temporarily admitted and the remaining Parents are rejected.

Step 2
• For each Parent who was rejected in the previous step, an application is sent to the Nursery that they ranked second on their "Choice Ranking List".
• Each Nursery that receives new applications considers the Parent it admitted in the previous step together with the new applicants. Among these, the Parent with the highest priority is temporarily admitted and the remaining Parents are rejected.

Following steps
• The procedure continues according to the same rules.

End of Part 1
• The procedure in Part 1 ends when no Parent is rejected.

Part 2
This part checks for "blocking Parents" based on the distinction between temporary admissions (which occur during the steps of Part 1) and final admissions (determined at the end of Part 1).
A Parent is identified as a blocking Parent at a Nursery if their situation corresponds to the following specific case:
1. Prevention: The Parent was temporarily admitted at a Nursery during the steps of Part 1, and this temporary admission caused other Parents to be rejected from that Nursery.
2. Discrepancy: However, this temporary admission differs from the Parent's final admission. That is, the Parent eventually moved to a different Nursery (or remained unmatched) by the end of Part 1.
In this case, the Parent's temporary presence "blocked" others from a seat that the Parent did not ultimately utilize.

Step 1
• The computer looks for the last step of the procedure in Part 1 in which a Parent has become a blocking Parent.
• If a Parent is a blocking Parent at a Nursery, the computer will automatically remove the respective Nursery from the Parent's "Choice Ranking List" and rerun the procedure described in Part 1.
• Note: This automatic waiver will never change your final admission but may improve other Parents' final admissions.

Step 2
• If the procedure has not ended (i.e., blocking Parents are still found), the procedure described in the previous step is repeated.

Final Step
• The procedure ends when there is no step in which a Parent becomes a blocking Parent. The admissions at this point are final.
"""
...
def main():
    ...
    if os.path.exists(quota_path):
        ...
        quotas = {c: 1 for c in pref_data["nurseries"]}
