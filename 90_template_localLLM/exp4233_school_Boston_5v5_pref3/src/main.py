...
from algorithm_engine import run_boston_algorithm
...
• Each High School considers all applications received. The Student with the highest priority is **permanently admitted** up to the High School's capacity.
• The remaining Students are rejected.
• Note: Once a Student is admitted in this step, their match is final. The High School is no longer available in subsequent steps.
...
• Importantly, Students can only apply to High Schools that still have seats available (i.e., High Schools that did not fill their position in Step 1).
• Among the new applicants, the available High Schools permanently admit the Students with the highest priority. The remaining Students are rejected.

Following steps
• The procedure continues according to the same rules, with rejected Students applying to their next ranked High School, provided that the High School still has a vacancy.

Final Step
• The procedure ends when no Student is rejected or all rejected Students have run out of High Schools on their list.
"""
