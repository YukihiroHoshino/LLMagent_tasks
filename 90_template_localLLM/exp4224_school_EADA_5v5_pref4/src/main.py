...
Part 1
Step 1
• For each Student, an application is sent to the High School that they ranked first on their "Choice Ranking List".
• If a High School receives only one application, the Student is temporarily admitted.
• If a High School receives more than one application, the Student with the highest priority (based on the High School's internal standards) is temporarily admitted and the remaining Students are rejected.

Step 2
• For each Student who was rejected in the previous step, an application is sent to the High School that they ranked second on their "Choice Ranking List".
• Each High School that receives new applications considers the Student it admitted in the previous step together with the new applicants. Among these, the Student with the highest priority is temporarily admitted and the remaining Students are rejected.

Following steps
• The procedure continues according to the same rules.

End of Part 1
• The procedure in Part 1 ends when no Student is rejected.

Part 2
This part checks for "blocking Students" based on the distinction between temporary admissions (which occur during the steps of Part 1) and final admissions (determined at the end of Part 1).
A Student is identified as a blocking Student at a High School if their situation corresponds to the following specific case:
1. Prevention: The Student was temporarily admitted at a High School during the steps of Part 1, and this temporary admission caused other Students to be rejected from that High School.
2. Discrepancy: However, this temporary admission differs from the Student's final admission. That is, the Student eventually moved to a different High School (or remained unmatched) by the end of Part 1.
In this case, the Student's temporary presence "blocked" others from a seat that the Student did not ultimately utilize.

Step 1
• The computer looks for the last step of the procedure in Part 1 in which a Student has become a blocking Student.
• If a Student is a blocking Student at a High School, the computer will automatically remove the respective High School from the Student's "Choice Ranking List" and rerun the procedure described in Part 1.
• Note: This automatic waiver will never change your final admission but may improve other Students' final admissions.

Step 2
• If the procedure has not ended (i.e., blocking Students are still found), the procedure described in the previous step is repeated.

Final Step
• The procedure ends when there is no step in which a Student becomes a blocking Student. The admissions at this point are final.
"""
...
def main():
    ...
    if os.path.exists(quota_path):
        ...
        quotas = {c: 1 for c in pref_data["schools"]}

    job_seekers_true = pref_data["students"]
    companies_true = pref_data["schools"]
    
    simulator = AgentSimulator(model="BEFORE_MODEL_NAME", temperature=0.7)
