...
Ordering Phase
• At the beginning, the computer assigns a random serial order to all Parents (e.g., 1st, 2nd, 3rd...). This order is determined purely by chance and is unrelated to the Nurseries' internal standards.

Selection Procedure
• The computer calls Parents one by one according to their assigned Serial Order.

Step 1
• The Parent with the 1st Serial Order is assigned to the Nursery ranked highest on their "Choice Ranking List".

Step 2
• The Parent with the 2nd Serial Order is assigned to the Nursery ranked highest on their "Choice Ranking List", strictly among the Nurseries that have not yet been taken by the previous Parent.

Following steps
• The procedure continues sequentially. Each Parent is assigned to their highest-ranked Nursery that is still available (i.e., not taken by Parents with an earlier Serial Order).
• If all Nurseries on a Parent's list are already taken, they remain unmatched.

Final Step
• The procedure ends after the last Parent in the Serial Order has been processed. All assignments are final.
"""
